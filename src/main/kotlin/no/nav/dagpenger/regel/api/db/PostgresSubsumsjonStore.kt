...
internal class PostgresSubsumsjonStore(private val dataSource: DataSource) : SubsumsjonStore, HealthCheck {
        ...
        withTimer<Int>("insertBehov") {
            return try {
                using(sessionOf(dataSource)) { session ->
                    ...
                    |                  VALUES (:id, :behandlings_id, :aktor, :beregning, :fisk, :verneplikt, :forste, :siste, :barn, :grunnlag, :forrige_grunnlag, :inntekt, :laerling, :regelverksdato, :data)
                                """.trimMargin(),
                                mapOf(
    ...
    override fun getBehov(behovId: BehovId): InternBehov =
        withTimer<InternBehov>("getBehov") {
    ...
    override fun getSubsumsjon(behovId: BehovId): Subsumsjon =
        withTimer<Subsumsjon>("getSubsumsjon") {
            val json =
                using(sessionOf(dataSource)) { session ->
                    session.run(
    ...
    private inline fun <reified R : Any?> withTimer(
        metric: String,
        block: () -> R,
    ): R {
